...
const c_print_expression_mod = @import("c_print_expressions.zig");
...
pub fn processPrintF(allocator:Allocator, convert_data:*ConvertData, node:*ASTNode) !void {
    ...
    for (0..child_count) |child_index| {
        ...
        if (convert_data.compiler_settings.separate_expressions) {
            ...
            final_value = try c_print_expression_mod.printExpression(allocator, convert_data, child, false);
        }
    ...
    convert_data.generated_code.append(allocator, "printf(") catch return ConvertError.Out_Of_Memory;
...
pub fn processPrint(allocator:Allocator, convert_data:*ConvertData, node:*ASTNode, new_line:bool) !void {
    ...
    if (child_count == 0) {
        ...
        if (new_line == true) {
            convert_data.generated_code.append(allocator, "std::cout('\\n')\n\t") catch return ConvertError.Out_Of_Memory;
            try convert_data.addNLWithTabs(allocator);
        }
        return;
    }
    ...
    convert_data.generated_code.append(allocator, "std::cout") catch return ConvertError.Out_Of_Memory;
