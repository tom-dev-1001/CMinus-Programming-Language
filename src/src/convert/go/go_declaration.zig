...
pub fn processDeclaration(allocator:Allocator, convert_data:*ConvertData, node:*ASTNode, add_new_line:bool, add_tabs:bool) ConvertError!void {
    ...
    var go_type:?[]const u8 = go_utils_mod.convertToGoType(type_token);

    if (go_type == null) {
        go_type = type_token.Text;
    }
    ...
    const value:[]const u8 = try print_expression_mod.printExpression(allocator, convert_data, node.right.?, false);
...
fn processDeclarationWithFlatten(allocator:Allocator, convert_data:*ConvertData, node:*ASTNode) ConvertError!void {
    ...
    const left:*ASTNode = node.left.?;
    const type_token:Token = left.token.?;
    const go_type:?[]const u8 = go_utils_mod.convertToGoType(type_token);
